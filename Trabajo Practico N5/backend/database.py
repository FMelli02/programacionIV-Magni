...
def update(participante_id: int, datos: ParticipanteUpdate) -> ParticipanteResponse | None:
    """Actualiza los campos enviados de un participante. Retorna el registro actualizado o None si no existe."""
    # Construimos el SET dinámicamente: solo pisamos los campos que no son None
    campos = datos.model_dump(exclude_none=True)
    if not campos:
        # Nada para actualizar: retornamos el registro actual
        with get_connection() as conn:
            row = conn.execute("SELECT * FROM participantes WHERE id = ?", (participante_id,)).fetchone()
            return row_to_participante(row) if row else None

    # Serializar tecnologias si viene en el payload
    if "tecnologias" in campos:
        campos["tecnologias"] = json.dumps(campos["tecnologias"])
    if "aceptaTerminos" in campos:
        campos["aceptaTerminos"] = int(campos["aceptaTerminos"])

    set_clause = ", ".join(f"{col} = ?" for col in campos.keys())
    valores = list(campos.values()) + [participante_id]

    with get_connection() as conn:
        cursor = conn.execute(
            f"UPDATE participantes SET {set_clause} WHERE id = ?",
            valores,
        )
        conn.commit()
